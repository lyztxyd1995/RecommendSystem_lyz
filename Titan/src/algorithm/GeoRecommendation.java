...
public class GeoRecommendation {
	 public List<Item> recommendItems(String userId, double lat, double lon) {
            ...
            //get users' all favorite items and recommend according to items' categories.
		    ...
		    //search all elements that have the same categories nearby
		    for (String category : allCategories) {
		      List<Item> items = conn.searchItems(userId, lat, lon, category);  // call external API
		      recommendedItems.addAll(items);
		    }

		    // filter the recommendedItems, remove item that already exit in favorite items
		    ...
		    Collections.sort(filteredItems, new Comparator<Item>() {
		    	 //create new comparator that compare items by distance.
		      @Override
